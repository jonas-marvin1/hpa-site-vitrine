	...
	if (isset($_POST["submit"])) 
	{
			...
			if (($cv && !in_array($cv_ext, $allowed_exts)) || ($diplome && !in_array($diplome_ext, $allowed_exts))) {
				...
				if( ($cv && move_uploaded_file($tmp, $repertoire)) || ($diplome && move_uploaded_file($tmpp, $repertoiree)) || isset($nom) ){
					$stmt = mysqli_prepare($con, "INSERT INTO `preinscription` (`nom`, `prenom`, `contact`, `mail`, `sexe`, `nationalite`, `fonction`, `niveau_etude`, `formation_choisir`, `hpa_connaitre`, `diplome`, `cv`, `commentaire`, `createdon`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, current_timestamp())");
					mysqli_stmt_bind_param($stmt, "sssssssssssss", $nom, $prenom, $contact, $mail, $sexe, $nationalite, $fonction, $etude, $certificat, $hpa, $diplome_new_name, $cv_new_name, $commentaire);
...
<head>
  ...
  <meta name="theme-color" content="#ffffff">
    ...
    <title>High Performance Academy - Certificats</title>
</head>
<body>
 ...
 <nav class="navbar navbar-expand-xl navbar-light bg-white sticky-top">
        ...
        <!-----------a propos ---------------->
         ...
         <!-------------- certificats  --------------->
                        ...
                        <li class="nav-item dropdown">
                            <a href="#" class="nav-link dropdown-toggle active text-nowrap" data-bs-toggle="dropdown">Certificats</a>
                            <div class="dropdown-menu" style="max-width: max-content;">
...
<section class="formation" id="formations">
  <div class="titre-formation">
    <h1>Nos <span>C</span>ertificats</h1>
      <p>Nos formations certifiantes débouchent sur des certificats professionnels et sont disponibles en ligne et en présentiel. </p>
  </div>
                ...
                <div class="box">
                    ...
                    <div class="text">
                        <h4 style="font-weight: bold; line-height: 25px;">
                            <?php echo $row['hpa']; ?> <br><small style="font-size: 14px; color: #555;">(<?php echo $row['libelle']; ?>)</small></h4>
